...
//            featureString = vd.getStringRepresentation();
    ...
    public Document createDocument(byte[] feature, String face_id) {
        ...
        if (face_id != null) {
            doc.add(new StringField(DocumentBuilder.FIELD_NAME_IDENTIFIER, face_id, Field.Store.YES));
        }

        Field[] fields = createDescriptorFields(feature);
        for (int i = 0; i < fields.length; i++) {
            doc.add(fields[i]);
        }
    ...
    public Field[] createDescriptorFields(byte[] feature) {
        Field[] result;
        if (hashingEnabled) result = new Field[2];
        else result = new Field[1];
        String featureString = "";
        ...
        try {
            ...
            // TODO: Stored field is compressed and upon search decompression takes a lot of time (> 50% with a small index with 50k images). Find something else ...
            result[0] = new StoredField(fieldName, new BytesRef(feature));
