...
@Service
public class SolrDataServerImpl implements SolrDataServiceItf {

    private static Logger LOG = LogManager.getLogger(SolrDataServerImpl.class);
...
//  @Override
//  @ReadThroughSingleCache(namespace = "face_search_bycamera", expiration = 600)
//  public TreeMap<Long, List<FaceResultDto>> searchFaceByFaceIdInAll(@ParameterValueKeyProvider String key, String faceFeature, float scoreThreshold, int type) throws Exception {
...
//      List<FaceResultDto> pojos = searchFaceByFaceIdInBank(key, faceFeature, scoreThreshold, type);
//      if(type == 0) {
...
//  public List<FaceResultDto> searchFaceByType(@ParameterValueKeyProvider String key, String faceFeature, float scoreThreshold, int type) throws Exception {
//      SolrQuery query = new SolrQuery();
//      query.set("iff","true");
//      query.set("threshold", scoreThreshold+"");
//      query.set("fq","type:"+type);
...
//      query.addFilterQuery("type:"+type+" AND time:[* TO 2016-01-27T16:00:00Z]");
        ...
        List<FaceResultDto> filterResultList = resultList;
        ...
        if(type < 1 || type > 2) {
            ...
            ObjectMapper objectMapper = new ObjectMapper();
            List<FaceResultDto> pojos = objectMapper.convertValue(namedlist.get(RESULT_DOCS), new TypeReference<List<FaceResultDto>>() { });
            if(null != pojos)  resultList.addAll(pojos);
                ...
                threadPool.submit(new Runnable() {
                    public void run() {
                        try {
                            ...
                            List<FaceResultDto> pojos = objectMapper.convertValue(namedlist.get(RESULT_DOCS), new TypeReference<List<FaceResultDto>>() { });
                            if(null != pojos)  resultList.addAll(pojos);
                            logBuffer.append("  "+faceFeature.substring(0, 100)+" | "+server.getBaseURL() + " ---> "+(System.currentTimeMillis()-now)+" ms"+"\r\n");
                        } catch (Exception e) {
                            e.printStackTrace();
                        } finally {
                            finishList.add(1);
            ...
            while(finishList.size()<num) {
                ...
                filterResultList = zoneAuthorizeService.filterById(CameraInfo.class, resultList, null);
                }
...
//      Collections.sort(resultList, new FaceResultDtoComparable("time"));
        LOG.info("search result num:"+filterResultList.size());
        return filterResultList;
    }
    
    @Override
    @ReadThroughSingleCache(namespace = "face_search_by_attribute", expiration = 600)
    public List<FaceResultDto> searchFaceByAttribute(@ParameterValueKeyProvider final String key, final String faceFeature, SearchFaceDto sfd) throws Exception {
                ...
                threadPool.submit(new Runnable() {
                    public void run() {
                        try {
                            ...
                            NamedList<Object> namedlist = rsp.getResponse();
                            ObjectMapper objectMapper = new ObjectMapper();
                            List<FaceResultDto> pojos = objectMapper.convertValue(namedlist.get(RESULT_DOCS),
                                    new TypeReference<List<FaceResultDto>>() {
                                    });
                            if(null != pojos)  resultList.addAll(pojos);
                            logBuffer.append(sdf.format(new Date()) + " " + faceFeature.substring(0, 100) + " | "
                                    + server.getBaseURL() + " ---> " + (System.currentTimeMillis() - now) + " ms"
                                    + "\r\n");
                        } catch (Exception e) {
                            e.printStackTrace();
                        } finally {
...
//      Collections.sort(resultList, new FaceResultDtoComparable("time"));
        ...
        LOG.info("search result num:"+filterResultList.size());
        if(filterResultList.size() > maxSize) {
            Collections.sort(filterResultList, new FaceResultDtoComparable(sfd.getSort()));
            List<FaceResultDto> tempList =  resultList.subList(0, maxSize);
            return tempList;
...
//      Collections.sort(resultList, new FaceResultDtoComparable("time"));
    ...
    public List<FaceResultDto> searchFaceByTime(@ParameterValueKeyProvider String key, String faceFeature, SearchFaceDto sfd) throws Exception {
        int type = sfd.getType();
        FileUtil.log("key:"+key+" type:"+type+" 未使用缓存!");
...
//      query.addFilterQuery("type:"+type+" AND time:["+searchStartTime+"Z TO "+searchEndTime+"Z]");
        ...
        if(type < 1 || type > 2) {
            ...
            // 离线布控不需要区域授权
            // filterResultList =
            // zoneAuthorizeService.filterById(CameraInfo.class, resultList);
        }

        // Collections.sort(filterResultList, new
        // FaceResultDtoComparable("time"));
        LOG.info("search result num:" + filterResultList.size());
        return filterResultList;
    }
    ...
    @Override
	@ReadThroughSingleCache(namespace = "face_search_by_camera", expiration = 600)
	public List<FaceResultDto> searchFaceByCamera(@ParameterValueKeyProvider String key, long cameraId, String faceFeature, float scoreThreshold, int type,int hours) throws Exception {
		FileUtil.log("key:"+key+" type:"+type+" 未使用缓存!");
...
//		query.set("fq","camera:"+cameraId);
		query.set("feature",faceFeature);
		query.set("featureAmount",faceFeature.split(";").length);
		query.set("rows",maxSize);
//		query.addFilterQuery("camera:"+cameraId+" AND time:[NOW-240MINUTE TO *]");

		LocalDateTime nowTime = LocalDateTime.now();
        String leftTime = nowTime.minusHours(8+hours).format(DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss'Z'"));
        String rightTime = nowTime.minusHours(8).format(DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss'Z'"));
        String tmep = "camera:" + cameraId + " AND time:[" + leftTime + " TO " + rightTime + "]";
        query.addFilterQuery(tmep);

        final List<FaceResultDto> resultList = new ArrayList<FaceResultDto>();
        final List<Integer> finishList = new ArrayList<Integer>();
        int num = 0;
...
//          final String faceFeature = blackDetail.getBase64FaceFeature();
    ...
    @ReadThroughSingleCache(namespace = "chd_face_search_by_type", expiration = 600)
    public List<FaceResultDto> chdSearchFaceByType(@ParameterValueKeyProvider final String key, final String faceFeature, float scoreThreshold, int type)
        ...
        LOG.debug(logBuffer.toString());
        // Collections.sort(resultList, new FaceResultDtoComparable("time"));
        LOG.info("search result num:" + resultList.size());
