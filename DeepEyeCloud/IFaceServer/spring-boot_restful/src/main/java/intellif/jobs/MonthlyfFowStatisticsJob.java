 ...
 * @author Linzhi.Wang
 * @version V1.0
 * @Title: MonthlyfFowStatisticsJob.java
 * @Package intellif.jobs
 * @Description
 * @date 2018 05-07 16:47.
 */
@Component
public class MonthlyfFowStatisticsJob {
	private static Logger logger = LogManager.getLogger(MonthlyfFowStatisticsJob.class);
	@Autowired
	...
	public void monthlyfFowStatistics() {
		logger.log(Level.INFO, ("每月统计定时任务开始---:"));
		Date now = new Date();
		...
		sqlString1.append("select CONCAT(DATE_FORMAT(time,   '%Y-%m'),'-01') as time,sum(count) as count,0 as age,0 as gender,person_type,20 as time_type,source_id");
		...
		sqlString1.append(" group by source_id,DATE_FORMAT(time,   '%Y-%m') order by DATE_FORMAT(time,   '%Y-%m')");
		...
		sqlString2.append("select CONCAT(DATE_FORMAT(time,   '%Y-%m'),'-01') as time,sum(count) as count,0 as age,gender ,person_type,20 as time_type,source_id");
		...
		sqlString2.append(" group by source_id,DATE_FORMAT(time,   '%Y-%m'),gender order by DATE_FORMAT(time,   '%Y-%m')");
		...
		sqlString3.append("select CONCAT(DATE_FORMAT(time,   '%Y-%m'),'-01') as time,sum(count) as count,age,0 as gender,person_type,20 as time_type,source_id");
		...
		sqlString3.append(" group by source_id,DATE_FORMAT(time,   '%Y-%m'),age order by DATE_FORMAT(time,   '%Y-%m')");
		//每月的总榜统计
		...
		logger.log(Level.INFO, ("每月统计定时任务结束---:"));
	}
}
