...
public class PoiExcelUtils {
	 ...
	 * contextStyle 内容样式
	 * serialNumFlag 是否添加序号
	 */
	public static <T> void addContextByList(HSSFSheet sheet, List<T> list, String[] fieldName, HSSFCellStyle contextStyle, boolean serialNumFlag) {
	 ...
	 *              contextStyle  表头样式
	 */
	public static void addTitle(HSSFSheet sheet, String[] assettitle, String titleName, HSSFCellStyle headerStyle, HSSFCellStyle contextStyle, boolean serialNumFlag) {
		sheet.addMergedRegion(new CellRangeAddress(0, 0, 0, serialNumFlag?assettitle.length:assettitle.length - 1));
		...
		for (int i = 0; i < assettitle.length; i++) {
			cell = row.createCell(serialNumFlag?i+1:i);
