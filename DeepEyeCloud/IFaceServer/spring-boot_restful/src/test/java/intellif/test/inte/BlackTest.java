...
public class BlackTest {
    ...
    public void setUp() {
		if ("".equals(accessToken) || null == accessToken) {
			accessToken = login("shixiaohua", "9f9c86d452bbe19c74533259fadc35fc");
		}
	...
	@Test
	public void testFindBlackByBanksPage() throws Exception {
		RequestBuilder requestBuilder = MockMvcRequestBuilders.get(GlobalConsts.R_ID_BLACK_DETAIL + "/bank/page/1/pagesize/1/personsize/1")
				.header("Authorization", "Bearer " + accessToken);
		ResultActions resultActions = this.mockMvc.perform(requestBuilder);
        resultActions.andExpect(status().is(HttpStatus.SC_OK));
	}
	...
	//@Test
	public void testCountBlackAndAuthorize() throws Exception {
		RequestBuilder requestBuilder = MockMvcRequestBuilders.get(GlobalConsts.R_ID_PERSON_DETAIL + "/black/authorize/count")
				.header("Authorization", "Bearer " + accessToken);
		ResultActions resultActions = this.mockMvc.perform(requestBuilder);
        resultActions.andExpect(status().is(HttpStatus.SC_OK));
	}
