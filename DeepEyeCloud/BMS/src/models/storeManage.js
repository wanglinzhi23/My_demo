...
import { create, remove, update ,queryList,removeList } from 'services/storeServices'
...
export default modelExtend(pageModel, {
      ...
      history.listen((location) => {
        if (location.pathname === '/storeManage') {
          if(JSON.stringify(location.query) == "{}"){//判断是否为空对象
            dispatch({
              type: 'query',
              payload: {searchName: "",userId: JSON.parse(localStorage.getItem("userInfo")).id,page: "1",pageSize: "10",},
            })
          }else{
            dispatch({
              type: 'query',
  ...
  effects: {
    ...
    * query ({ payload = {} }, { select, call, put }) {
      const { storeListParam } = yield select(_ => _.store)
      const param = {...storeListParam,...payload,};
    ...
    * delete ({ payload }, { call, put, select }) {
      ...
      const { selectedRowKeys } = yield select(_ => _.store)
